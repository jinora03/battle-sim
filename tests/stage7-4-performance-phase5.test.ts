...
describe("v1.1 Stage 7.4 performance phase 5", () => {
  ...
  it("keeps critical and player-related events inside the mass-battle visual budget", () => {
    ...
    expect(
      selected.some((event) => event.type === "damage" && event.sourceId === 1),
    ).toBe(true);
  ...
  it("prioritizes player projectiles while sampling the remaining projectile visuals", () => {
    ...
    expect(
      selected.filter((projectile) => projectile.sourceId === 1),
    ).toHaveLength(4);
  });
  ...
  it("boots Pixi only after the Fight workspace has a visible layout", () => {
    ...
    expect(runtimeHook).toContain("if (view !== 'battle')");
    expect(runtimeHook).toContain("await afterVisibleLayout()");
    expect(runtimeHook).toContain("runtimeBootRef.current ?? runtime.start()");
    expect(runtimeHook).not.toContain("void runtime.start()");
  });
});

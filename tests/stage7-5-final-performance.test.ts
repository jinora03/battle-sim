...
describe("v1.1 Stage 7.5 final performance architecture", () => {
  it("keeps team-grouped action selection deterministic and exact", () => {
    ...
    const baseline = selectAbilityAction(
      snapshot,
      self,
      target,
      profile,
    ...
    const optimized = selectAbilityAction(
      snapshot,
      self,
      target,
      profile,
    ...
    expect(diagnostics.areaCandidateChecks).toBeLessThan(
      diagnostics.hostileQueries * snapshot.entities.length,
    );
  });

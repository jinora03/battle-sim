...
export function checksumSnapshot(snapshot: WorldSnapshot): string {
  ...
  for (const entity of snapshot.entities) {
    const resourceState = entity.resources && entity.resources.length > 0
      ? `|r:${entity.resources.map((resource) => `${resource.resourceId}:${resource.value.toFixed(5)}:${resource.maximum.toFixed(5)}`).join(',')}`
      : '';
    feed(`${entity.id}|${entity.fighterId}|${entity.team}|${entity.x.toFixed(5)}|${entity.y.toFixed(5)}|${entity.vx.toFixed(5)}|${entity.vy.toFixed(5)}|${entity.hp.toFixed(5)}|${entity.activeZoneIds.join(',')}|${entity.statuses.map((status) => `${status.statusId}:${status.stacks}:${status.remainingTicks}`).join(',')}${resourceState}|${entity.moduleIds.join(',')}|${entity.primaryAttackId}|${entity.weaponAttack?.weaponId ?? 'idle'}:${entity.weaponAttack?.phase ?? 'idle'}:${entity.weaponAttack?.remainingTicks ?? 0}`);
